...
SUPERGROUP "Registers" {
	...
	WORD_VALUE "F" {
		...
		MACRO "FeedRate" {
			 O_INCH "DECIMAL:3.4"  O_METRIC "DECIMAL:4.0" 
		}
	}
	WORD_VALUE "H" {
		COND_AND "G" "45-48" {
...
SUPERGROUP "M_Misc" {
	...
	WORD_VALUE "M" "6" {
		MACRO "ErrorMacro" {
			TEXT_OVERRIDE_VALUE "Автоматическая смена инструмента на станке не работает! "
		}
	}
	WORD_VALUE "M" "7" {
		MACRO "CoolantMist"
...
SUPERGROUP "G_Prep" {
	...
	WORD_VALUE "G" "53,58" {
		MACRO "ErrorMacro" {
			TEXT_OVERRIDE_VALUE "Неверная команда !!! (выход в нуль станка))"
		}
	}
	WORD_VALUE "G" "54-57" {
		MACRO "WorkCoord"
