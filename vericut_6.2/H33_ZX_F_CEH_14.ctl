...
MACRO_FILE "V:\\vericut_5.4_4\\function_33.cme"
CSS_MAX_RPM 1000
...
SUPERGROUP "Registers" {
	...
	WORD_VALUE "F" {
		...
		COND_AND "Z" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "Перемещение с нулевой подачей ( F0000 ) !!!"
			}
		}
		COND_AND "F" "0"
		COND_AND "X" {
...
SUPERGROUP "G_Prep" {
	...
	WORD_VALUE "G" "50" {
		COND_AND "G" "2 3" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "Неверное отключениение коррекции (отход только на линейном участке) !"
			}
		}
		MACRO "CutterCompOff"
	}
