...
MCD_SUB_FILES {
	MCD_FILE_NAME "V:\\vericut_6.2\\V-STAR (S840D)_6\\sin840d.sub"
}
...
SUPERGROUP "States" {
	...
	WORD_VALUE "G" "2" {
		COND_VAR_AND "190" "1" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE " G18 не отрабатывает в окружности! Пересчитать УП  линейно в UG !!!"
			}
		}
		COND_VAR_AND "190" "2" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE " G19 не отрабатывает в окружности! Пересчитать УП  линейно в UG !!!"
			}
		}
		MACRO "MotionCW"
	}
