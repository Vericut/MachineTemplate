...
SUPERGROUP "Registers" {
	...
	WORD_VALUE "B" {
		...
		COND_VAR_AND "397" "1" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "B={$}   Пределы перемещения  B  от  0 до 357.5 градусов!!! "
			}
			MACRO "CAxisMotion" {
				OVERRIDE_EXP "#5006"
			}
	...
	WORD_VALUE "C" {
		...
		COND_AND "DEF" "19.1" {
			...
			MACRO "RotationPlaneAngle1" {
				OVERRIDE_VALUE -45 TEXT_OVERRIDE_VALUE "X"
			}
			MACRO "RotationPlaneLocal2"
			MACRO "RotationPlaneAngle2" {
				OVERRIDE_EXP "#5005"TEXT_OVERRIDE_VALUE "Z"
			}
			MACRO "RotationPlaneLocal2"
			MACRO "RotationPlaneAngle1" {
				OVERRIDE_VALUE 45 TEXT_OVERRIDE_VALUE "X"
		...
		COND_VAR_AND "397" "1" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "C={$}   Пределы перемещения  C  от  0 до 357.5 градусов!!! "
			}
		}
		COND_AND "C" ">357.5"
		COND_VAR_AND "397" "1" {
	...
	WORD_VALUE "CP" {
		COND_AND "DR-"
		COND_VAR_AND "100" "-5400.001-(-99999.99)" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE " Суммарный угол спирали IPA = {#100} меньше минимального (-5400) !!!"
			}
		}
		COND_AND "DR-"
		COND_VAR_AND "100" "0.001-99999.99" {
			MACRO "ErrorMacro" {
		...
		COND_VAR_AND "100" "5400.001-99999.99" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "Суммарный угол спирали IPA = {#100} больше максимального 5400 !!!"
			}
		}
		COND_AND "DR+"
		COND_VAR_AND "100" "-0.001-(-99999.99)" {
	...
	WORD_VALUE "Z" {
		...
		COND_AND "Y" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "Неверный формат кадра!!!"
			}
			MACRO "EndProgram"
		}
		COND_AND "DEF" "7.1 7.2 7.3 7.4 7.5" {
...
EVENTS {
	TYPE CONVERSION_INIT {
		...
		MACRO "TurnOnOffCompTravelLimits" {
			OVERRIDE_VALUE 0 TEXT_OVERRIDE_VALUE "C"
		}
		MACRO "TurnOnOffCompTravelLimits" {
...
MACRO_MODALS {
	...
	NAME "mmB_AXIS_ROTARY_TYPE" VALUE "mmABSOLUTE_360"
