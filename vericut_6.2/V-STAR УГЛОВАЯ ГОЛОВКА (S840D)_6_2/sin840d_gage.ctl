...
MCD_SUB_FILES {
	MCD_FILE_NAME "V:\\vericut_6.2\\V-STAR УГЛОВАЯ ГОЛОВКА (S840D)_6_2\\sin840d.sub"
}
...
SUPERGROUP "M_Misc" {
	...
	WORD_VALUE "M" "3" {
		MACRO "ErrorMacro" {
			TEXT_OVERRIDE_VALUE "Неверное вращение шпинделя (М3-ПОЧС).  Изменить ( М4-ПРЧС) !!!"
		}
	}
	WORD_VALUE "M" "-3" {
		MACRO "SpindleMotionCCW"
...
EVENTS {
	TYPE CONVERSION_INIT {
		...
		MACRO "Cinci_5axisToolLengthComp" {
			OVERRIDE_VALUE 1 
		}
		MACRO "Unitize3DVector" {
			OVERRIDE_VALUE 0 
		}
