...
SUPERGROUP "Registers" {
	...
	WORD_VALUE "X" {
		...
		COND_VAR_AND "128" "1" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE "M91/M92 не работают при M128 !!!"
			}
		}
		COND_AND "M" "91" {
...
SUPERGROUP "G_Prep" {
	...
	WORD_VALUE "STAY" {
		COND_AND "SPATIAL"
		COND_VAR_AND "201" ">360" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE " Угол SPA = {#211} выходит за пределы значений (от -359.999 до 359.999)!!!"
			}
		}
		COND_AND "SPATIAL"
		COND_VAR_AND "202" ">360" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE " Угол SPB = {#212} выходит за пределы значений (от -359.999 до 359.999)!!!"
			}
		}
		COND_AND "SPATIAL"
		COND_VAR_AND "203" ">360" {
			MACRO "ErrorMacro" {
				TEXT_OVERRIDE_VALUE " Угол SPC = {#213} выходит за пределы значений (от -359.999 до 359.999)!!!"
			}
		}
		MACRO "NullMacro"
	}
...
MACRO_MODALS {
	...
	NAME "mmRTCP_USES" VALUE "mmGAGE_PIVOT_OFFSET"
